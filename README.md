...
工場のキラキラ／風系パッド（`ps-*` と `pf-flute-pad` / `pf-choir-air` / `pf-silk` など）は同じ仕組みで **上下オクターブを同時に**混ぜ、各声を**静的に 1–3 Hz**ずらす（基音はLFOで揺らさない。うねりは根音と ±oct の干渉）。薄いときは +7 と +24（ミッドBP付き）。`--layers octave,octave-down` / `--layers octave-up-2,fifth` で固定。
...
`dr-*` ドローンバンク（正弦サブ、暗いReese、低いスーパーソー、中空5度、遅いFM、ノイズBPランブル、クワイア、トレーラーブルーム、リバースホールド、水中、インダストリアル、遠いブラス、雷ベッドなど50種）も同じ。出力は `dist/dr-….wav`。TOMLは `presets/drone/`。**既定の長さは 120 BPM・4/4 の8小節ホールド（16秒）以上**。サブベッドは約41秒、シネマティックパッドドローンは約20秒。キャリアのサステインは高く、t=14秒でも聞こえる（0.4秒で消えるワンショットではない）。サブ床は C1–E1（MIDI 24–28）。パッド／スペース系は C4–A5（MIDI 60–81）でミッド以上。既存の `bd-*` / `sd-*` / `ld-*` / `fx-*` / `bs-*` / `pc-*` はそのまま。

`pf-*` 爽やかパッド（朝のコーラス、Juno風の軽い広がり、フルートパッド、クワイア空気、開いた5度／9度、リディアン、柔らかい長三和音など30種）も同じ。出力は `dist/pf-….wav`。TOMLは `presets/pad-fresh/`。**既定の長さは 130 BPM の4全音符（4 × 4 × 60/130 = 960/130 ≈ 7.385秒）。** ドローンがサブ／ランブルの床を担うので、こちらは **低域を厚くしない**。HP／カットオフでエネルギーはだいたい 150–200 Hz より上。音高は C3–C5（MIDI 48–72）。風系の `pf-flute-pad` / `pf-choir-air` / `pf-silk` などは `ps-*` と同じレンダー時 ±オクターブ重ね。既存の `dr-*` / `bd-*` / `sd-*` / `ld-*` / `fx-*` / `bs-*` / `pc-*` はそのまま。

`ps-*` キラキラパッド（クリスタル、ベルホールド、シマー、オルゴールパッド、氷の輝き、進化するFMスパークル、遅いコーラスシャインなど30種）も同じ。出力は `dist/ps-….wav`。TOMLは `presets/pad-sparkle/`。長さは `pf-*` と同じ **4全音符 @ 130 BPM（≈7.385秒）**。**基音はウインドシンセ系（長いアタック、音程の通ったトーン）。キラキラは上の部分音。** キック／サブではない。**音高は C4（MIDI 60）。ハイパスはサブだけ切り、C4 基音は残す。** `render` / `analyze` の `--layers auto` は同じ4OPを **±オクターブでもう一度レンダし、2·f0+2 Hz / 0.5·f0−2 Hz のように静的にずらして混ぜる**（全体をLFOで揺らさない。4OP内部の比2化でもない）。分析でまだ薄いときは +7 と +24 を足し、その声だけ 200–2.2 kHz 付近のバンドパスでミッドに置く。既存バンクは触らない。
...
`--note` / `--duration` / `--hz` / `--velocity` / `--sample-rate` / `--bit-depth` / `--layers` は `render` と同じ。指定すると全プリセットに同じ値がかかる。`--layers auto`（既定）は工場リードをオクターブ上、キラキラ／風系パッドを ±オクターブ（静的 1–3 Hz）、工場FXのピッチ系をオクターブ下スタック。ベースやキックは単音のまま。
...
`--output` は PNG パス。JSON は同じ stem。プリセット指定時は同じ stem の WAV も書く。`--layers` は `render` と同じ（工場リードは既定でオクターブ上、キラキラ／風系パッドは ±オクターブ、工場FXのピッチ系はオクターブ下。JSON の `render_layers` に実際に混ぜた声が出る）。
...
| `sd-rimshot` | リムショット（スティック側：短い胴＋乾いたクリック） |
...
| `ld-harpsi` | ハープシコード寄り |
| `fx-rev-cym` | クラシックなリバースシンバル（音量の逆減衰のみ） |
| `fx-rev-crash` | 明るいリバースクラッシュ（音量の逆減衰のみ） |
| `fx-rev-hat` | 短いリバースハット（音量の逆減衰のみ） |
| `fx-rev-cym-long` | 長いダークなリバースシンバル（3.8秒。音量のみ） |
| `fx-rev-cym-bright` | 短い明るいリバースシンバル（音量の逆減衰のみ） |
| `fx-rev-cym-dark` | 暗いリバースシンバル（ライド寄り。音量のみ） |
| `fx-rev-crash-metal` | 金属寄りのリバースクラッシュ（音量の逆減衰のみ） |
| `fx-rev-air` | エア寄りのリバースハット（音量の逆減衰のみ） |
| `fx-rev-cym-noise` | ノイズ寄りのリバースシンバル（音量の逆減衰のみ） |
| `fx-rev-splash` | 短いリバーススプラッシュ（音量の逆減衰のみ） |
| `fx-noise-hit` | ホワイト寄りの短いノイズヒット |
| `fx-noise-burst` | 少し長いノイズバースト |
| `fx-metal-crash` | 金属FMクラッシュ |
| `fx-glass-smash` | ガラス破砕（2–8 kHz の衝突＋破片。ハットではない） |
...
| `fx-sweep-bp` | バンドパス掃引 |
| `fx-formant-ah` | アー母音のFXヒット（フォルマント合成 + 5度コーラス） |
| `fx-formant-oh` | オー母音のFXヒット（フォルマント合成 + 5度コーラス） |
...
| `pc-rim` | 木寄りのリム（スネア胴なしの短いウッドクリック） |
| `pc-cowbell` | カウベル |
| `pc-clave` | クラべ |
| `pc-snap` | フィンガースナップ |
| `pc-snaps` | スナップの重ね |
...
| `pc-guiro` | ギロの短いスクレイプ |
| `pc-stick` | スティックのクリック |
| `dr-sine-sub` | 正弦サブベッド（E1≈41 Hz。長い床） |
| `dr-sub-octave` | サブ＋オクターブの正弦スタック（C1+C2、両方サブ帯） |
| `dr-rumble` | パルスの低ランブル |
...
| `dr-choir-low` | クワイア寄りの重ねサイン（C4ミッドパッド） |
| `dr-choir-dark` | 暗いクワイアパッドドローン（C4、短3度） |
...
| `dr-scifi-hum` | 暗いスペースシンセのハム（D5ミッド） |
| `dr-horror` | ホラーの暗いスペースドローン（C#5ミッド） |
| `dr-ambient-dark` | 暗いアンビエントパッドドローン（C4ミッド） |
| `dr-brass-pad` | 低いブラスパッド |
| `dr-brass-distant` | 遠い金管 |
| `dr-thunder-bed` | 雷のベッド（遠雷ノイズ＋サブ） |
| `dr-dystopia` | ディストピアのハム |
| `dr-pad-dark` | 暗いシネマティックパッドドローン（C4ミッド） |
| `dr-hum-grid` | 50/60 Hzの電源グリッド |
| `dr-void` | 虚空（E1正弦だけの長いサブ） |
| `dr-abyss` | 深淵のサブランブル（E1＋薄い砂） |
| `dr-cathedral` | 聖堂の明るいドローバー（C5トレーラーホールド） |
| `dr-engine` | エンジン／タービンのミッドエア（~1.7 kHz） |
| `dr-reactor` | 原子炉／スペースのミッドハム（C5） |
| `dr-ice-cave` | 氷穴の明るいミッドドローン（C5） |
| `dr-fog` | 霧のパッドドローン（C4ミッド） |
| `dr-warfare` | 戦争映画の床 |
| `dr-ritual` | 儀式の低い重ね（5度＋短3度） |
| `dr-ghost-choir` | 幽霊クワイア（C5の明るいパッド） |
| `dr-metal-bed` | 金属ベッド |
...
| `dr-storm` | 嵐のランブル |
| `dr-score-hold` | スコア／トレーラーの明るいホールド（C5） |
...
440 Hz 正弦の重心、ノイズの平坦度、ピッチ落下、`bd-808-boom` のサブと落下、PNG 寸法と JSON のパース、エンジンが無音でないこと、WAVヘッダとデータサイズ、工場プリセットのスモーク、`bd-*` キックと `sd-*` スネアがそれぞれちょうど20個で非無音、`ld-*` リードと `fx-*` FXがそれぞれちょうど50個で非無音、工場リードのレンダー時オクターブ重ね（`ld-fm-pluck` はアルゴリズム1のまま、遅延部の 2×f0 が第二声であること）、`bs-*` ベースがちょうど15個で非無音、`pc-*` パーカッションがちょうど50個で非無音、`dr-*` ドローンがちょうど50個で非無音、`pf-*` 爽やかパッドと `ps-*` キラキラパッドがそれぞれちょうど30個で非無音、`pl-*` プラックがちょうど30個で非無音かつ短い（既定は2秒未満。`pl-reverse-swell` だけ例外）、`ep-*` エレクトリックピアノがちょうど5個で非無音かつ1.2秒超（クリックではない）、`ep-rhodes-soft` のアタックに純正弦より強い2×/3×タインがあること、`presets/ld/` の既定レンダーが約5.54秒（130 BPM の3全音符）で末尾0.4秒が無音でないこと、`dr-*` の既定レンダーが16秒以上（120 BPM の8小節、サブ／パッドベッドはより長い）で末尾1秒と t=14秒が無音でないこと、`pf-*` / `ps-*` の既定レンダーが約7.385秒（130 BPM の4全音符）で末尾と t=0.85 が無音でないこと、`render_all_factory` が工場IDの数だけ非無音WAVを出すこと、super-saw が正弦と違うこと、低いLPカットオフが高域を落とすことを見る。WAVは `/tmp/fm_synth_tests/` など一時ディレクトリへ出す（リポジトリの `dist/` には書かない）。
